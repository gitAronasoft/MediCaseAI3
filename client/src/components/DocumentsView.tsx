...
import { FileText, Upload, Eye, Brain, Download, CheckCircle } from "lucide-react";
...
export default function DocumentsView() {
  ...
  const setDocumentObjectMutation = useMutation({
    mutationFn: async ({ documentId, objectURL }: { documentId: string; objectURL: string }) => {
      const response = await apiRequest("PUT", `/api/documents/${documentId}/object`, { objectURL });
      return response.json();
    },
  });
  ...
  const handleUploadComplete = async (result: UploadResult<Record<string, unknown>, Record<string, unknown>>) => {
    if (result.successful && result.successful.length > 0 && selectedCaseId) {
      ...
      const documentData = {
        ...
        objectPath: "", // Will be set after ACL policy is applied
      };

      const document = await createDocumentMutation.mutateAsync(documentData);

      // Set object ACL policy and get normalized path
      await setDocumentObjectMutation.mutateAsync({
